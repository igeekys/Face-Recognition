...
def Global_Average_Pooling(x):
    ...
    # return tf.keras.layers.GlobalAvgPool2D(x, name='Global_avg_pooling')
    gap = tf.reduce_mean(x, axis=[1, 2], keepdims=True, name='Global_avg_pooling')
    return gap
...
class SE_Inception_resnet_v2():
    def __init__(self, x, Nclass, is_training):
    ...
    def Build_SEnet(self, input_x):
        ...
        x = Fully_connected(x, units=self.Nclass, layer_name='final_fully_connected')
        return x
