...
def classification_loss(logit, label, top_k) :
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=label, logits=logit))
    prediction = tf.equal(tf.argmax(logit, -1,name='classes'), tf.argmax(label, -1))
    accuracy = tf.reduce_mean(tf.cast(prediction, tf.float32))

    top_k_op = tf.nn.in_top_k(logit, tf.argmax(label, -1), top_k)
    top_k_acc = tf.reduce_mean(tf.cast(top_k_op, tf.float32))
